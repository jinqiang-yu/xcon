...
class Cactus(Plot, object):
    ...
    def create(self, data):
        ...
        lines = plt.plot(*coords, zorder=3)
        #lines = plt.plot(*coords, zorder=3, linewidth=1, markersize=1)
        ...
        for i, l in enumerate(lines):
            ls = self.linestyles[lmap(i) % len(self.linestyles)]
            ls['ms'] = self.ms
            ls['lw'] = self.width
            ls['mew'] = self.width * 0.75
            plt.setp(l, **ls)
            #self.linestyles[lmap(i) % len(self.linestyles)])
