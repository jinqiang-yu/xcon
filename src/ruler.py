...
class Ruler(object):
    ...
    def prepare_formula(self):
        ...
        self.vdrmap = {value: key for key, value in self.drvmap.items()}
    ...
    def coverage(self):
        ...
        if self.options.order == 'sup':
            wght = 1
            for tvar in self.tvars:
                self.formula.soft.append([tvar])
            self.formula.wght = [wght for cl in self.tvars]
    ...
    def enumerate_sorted(self):
        ...
        def cutp(feats):
            cut = False
            if self.options.cutp < 1:
                if feats / len(self.data.names) > self.options.cutp:
                    cut = True
            else:
                if feats > self.options.cutp:
                    cut = True
            return cut
        ...
        if self.tvars:
            for mod in self.rc2.enumerate():
                ...
                fired_insts = list(filter(lambda ll : ll in mod, items))
                len_ops_insts = len(self.data.samps) - len(items)
                all_feats = len(self.data.names)
                feat_invl = len(mcs) + 1

                conditions = str(rule).split('=>')[0].split(',')
                conditions = list(map(lambda l: l.replace("'", "").strip(), conditions))
                conditions = list(map(lambda l: l.replace(': ', ' == ') if 'not ' not in l
                                            else l.replace(': ', ' != ').replace('not ', ''), conditions))
                conditions = 'IF ' + ', '.join(conditions)
                predict = ' THEN' + str(rule).split('=>')[-1].replace("'", "").replace(': ', ' == ')
                srule = conditions + predict

                self.rule_stat[srule] = {}
                self.rule_stat[srule]['feat_invl'] = feat_invl
                self.rule_stat[srule]['all_feats'] = all_feats
                self.rule_stat[srule]['fired'] = len(fired_insts)
                self.rule_stat[srule]['samelbl'] = len(items)
                self.rule_stat[srule]['firedopp'] = len(fired_insts) + len_ops_insts
                self.rule_stat[srule]['all'] = len(self.data.samps)
