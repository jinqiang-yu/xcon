...
class Data(object):
    ...
    def __init__(self, filename=None, fpointer=None, dataframe=None, lines=None,
            names=None, mapfile=None, separator=',', ranges=None, rextract=None, test=False):
        """
        ...
        if filename:
            if filename.endswith('.gz'):
                with gzip.open(filename, 'rt') as fp:
                    self.parse_fp(fp, separator, rextract, test)
            else:
            ...
            self.parse_fp(fp=None, separator=separator, rextract=rextract, test=test, lines=lines)

        if test is True or lines is not None:
            self.process_data(rextract, test=True)
        else:
            self.process_data(rextract, test=False)

        if self.mname:
            self.read_orig_values()

    def parse_fp(self, fp, separator, rextract, test, lines=None):
        """
        ...
        if rextract or test:
            # dropping class labels if data used for rule mining or computing explanations
            #lines = [','.join(line.strip().split(separator)[:-1]) for line in lines]
            lines = [','.join(line.strip().split(separator)[ : len(self.names) - 1]) for line in lines]

            # reading preamble
            self.names.pop()

            if rextract:
                # move selected feature to last position
                self.names.append(self.names.pop(rextract - 1))
        ...
        for line, w in six.iteritems(collections.Counter(lines)):
            ...
            sample = sample[:len(self.names)-1] + [','.join(sample[len(self.names) - 1 :])]
            if rextract:
                # move selected feature to last positio
                sample.append(sample.pop(rextract-1))
    ...
    def process_data(self, rextract, test):
        """
        ...
        if test or rextract:
            fvars = len(self.names)
        else:
            fvars = len(self.names) - 1

        for i in range(fvars):
        ...
        if not test:
            ...
            feats = sorted(self.feats[-1])
            if not rextract:
                for l in feats:
                    self.fvmap.dir[(self.names[-1], l)] = next(idpool)
            else:
                # if number distinct values != 2, all labels have been marked with distinct ids
                if len(feats) == 2:
                   self.fvmap.dir[(self.names[-1], feats[1])] = next(idpool)
    ...
    def __iter__(self):
        ...
        for samp in self.samps:
            yield tuple(map(lambda f:  '='.join(list(self.fvmap.opp[f])), samp))
