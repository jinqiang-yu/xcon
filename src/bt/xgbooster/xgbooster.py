...
class XGBooster(object):
        ...
        if from_data:
            ...
            if (self.use_categorical):
                ...
                for i in self.categorical_features:
                    ...
                    # change to this in case not all feature values are encoded
    ...
    def encode(self, test_on=None):
        ...
        # print(self.extended_feature_names)
        # print(self.extended_feature_names_as_array_strings)
        # print(self.target_name)
        encoder = SMTEncoder(self.model, self.feature_names, self.num_class, self)
        self.enc, self.intvs, self.imaps, self.ivars, self.knowledge, self.fid2fv = encoder.encode()
        if test_on:
    ...
    def explain(self, sample):
        ...
        if 'x' not in dir(self):
            self.x = SMTExplainer(self.enc, self.intvs, self.imaps,
                    ...
                    self.options, self.knowledge, self.fid2fv, self)

        expl = self.x.explain(np.array(sample), self.options.smallest)

        # returning the explanation
        return expl, self.x.time, self.x.filter_time, self.x.use_time
    ...
    def validate(self, sample, expl):
        ...
        res, minimal, expl_ = self.x.validate(np.array(sample), expl)
