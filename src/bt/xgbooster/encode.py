...
class SMTEncoder(object):
    ...
    def encode(self):
        ...
        self.knowledge = []
        self.fid2fv = {}
        if self.optns.knowledge and self.optns.dtinfo:
            self.encode_bg()
        ...
        return self.enc, self.intvs, self.imaps, self.ivars, self.knowledge, self.fid2fv
    ...
    def encode_bg(self):
        ...
        with open(self.optns.dtinfo, 'rb') as f:
        ...
        for lname in knowledge:
            for lvalue in knowledge[lname]:
                if lvalue.lower() == 'true':
                    ...
                    try:
                        label_value = int(lvalue)
                    except:
                        try:
                            label_value = float(lvalue)
                        except:
                            label_value = lvalue

                lf = dtinfo['feature_names'].to_list().index(lname)

                lid = dtinfo['categorical_names'][lf].tolist().index(label_value)
                lfid = 'f{0}_{1}'.format(lf, lid)

                if lfid not in self.fid2fv:
                    self.fid2fv[lfid] = tuple([lname, label_value])
                ...
                for imp in knowledge[lname][lvalue]:
                    ...
                    for finfo in imp:
                        ...
                        sign = finfo['sign']
                        f = dtinfo['feature_names'].to_list().index(feature)

                        id = dtinfo['categorical_names'][f].tolist().index(value)

                        fid = 'f{0}_{1}'.format(f, id)
                        bv = Symbol(fid, typename=BOOL)

                        if fid not in self.fid2fv:
                            self.fid2fv[fid] = tuple([feature, value])

                        prefix.append(bv if sign else Not(bv))

                    self.knowledge.append(Implies(And(prefix), Symbol(lfid, typename=BOOL)))