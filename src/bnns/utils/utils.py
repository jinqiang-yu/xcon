...
def save_checkpoint(state, is_best, path='.', filename=CHECKPOINT_MODEL_DEFAULT_FILE_NAME, save_all=False):
    filename = os.path.join(path, filename)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, os.path.join(path, BEST_MODEL_DEFAULT_FILE_NAME))
    if save_all:
...
def bce_accuracy(output, target):
    ...
    #print(batch_size)
    correct = (pred.long() == target.long()).float().sum()/ batch_size
    #print((pred.long() == target.long()).sum())
    #print(correct)
    return correct
